...
class EditProfileViewController: UIViewController {
    ...
    private let profilePlaceholderImageView: UIImageView = {
    ...
    override func viewDidLoad() {
        ...
        scrollView.addSubview(profilePlaceholderImageView)
        ...
        updateButton.addTarget(self, action: #selector(didTapSubmit), for: .touchUpInside)
        profilePlaceholderImageView.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(didTapToUpload)))
        //bindViews()
        
    }
    
    @objc private func didTapSubmit() {
        userProfileData.uploadProfile()
    }
    
    @objc private func didUpdateDisplayName() {
        userProfileData.displayName = displayNameTextField.text
        userProfileData.validateUserProfileForm()
    }
    
    @objc private func didUpdateUsername() {
        userProfileData.username = usernameTextField.text
        userProfileData.validateUserProfileForm()
    }
    ...
    private func bindViews() {
        ...
        userProfileData.$isFormValid.sink { [weak self] buttonState in
            self?.updateButton.isEnabled = buttonState
        }
        .store(in: &subscriptions)
        
        userProfileData.$isOnboarding.sink { [weak self] success in
            if success {
    ...
    private func configureConstraints() {
        ...
        let avatarPlaceholderImageViewConstraints = [
            profilePlaceholderImageView.centerXAnchor.constraint(equalTo: scrollView.centerXAnchor),
            profilePlaceholderImageView.heightAnchor.constraint(equalToConstant: 120),
            profilePlaceholderImageView.widthAnchor.constraint(equalToConstant: 120),
            profilePlaceholderImageView.topAnchor.constraint(equalTo: hintLabel.bottomAnchor, constant: 30)
        ]
        ...
        let displayNameTextFieldConstraints = [
            ...
            displayNameTextField.topAnchor.constraint(equalTo: profilePlaceholderImageView.bottomAnchor, constant: 40),
        ...
        let submitButtonConstraints = [
            ...
            updateButton.bottomAnchor.constraint(equalTo: view.keyboardLayoutGuide.topAnchor, constant: -20)
        ]
...
extension EditProfileViewController: UITextViewDelegate, UITextFieldDelegate {
    ...
    func textViewDidChange(_ textView: UITextView) {
        userProfileData.bio = textView.text
        userProfileData.validateUserProfileForm()
    }
            ...
            result.itemProvider.loadObject(ofClass: UIImage.self) { [weak self] object, error in
                if let image = object as? UIImage {
                    DispatchQueue.main.async {
                        self?.profilePlaceholderImageView.image = image
                        self?.userProfileData.imageData = image
                        self?.userProfileData.validateUserProfileForm()
                    }
                }
            }
        }
    ...
    func fetchUser() {
        ...
        Firestore.firestore().collection("users").document(userId).getDocument { snapshot, error in
            if error != nil {
                ...
                self.profilePlaceholderImageView.sd_setImage(with: URL(string: snapshot?.get("profilePath") as! String))
                self.usernameTextField.text = snapshot?.get("userName") as? String
                self.displayNameTextField.text = snapshot?.get("displayName") as? String
                self.bioTextView.text = snapshot?.get("bio") as? String
